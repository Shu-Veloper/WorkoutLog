...
3.2 달력 페이지 (Calendar View) - 일반 달력 뷰
기능
...
```
┌─────────────────────────────────────────────────┐
│           < 2025년 1월 >                        │  ← 월 선택 (일반 달력과 동일)
├─────────────────────────────────────────────────┤
│                                                 │
│  1일 (수) │ [오늘 컨디션 좋음!_______] [저장]   │
│  2일 (목) │ [휴식일________________] [삭제]    │  ← 값 있으면 삭제 버튼
│  3일 (금) │ [____________________] [저장]      │  ← 빈 값은 저장 버튼
│  4일 (토) │ [등 운동 1시간 완료______] [삭제]   │
│  5일 (일) │ [____________________] [저장]      │
│  ...                                            │
│  31일 (금)│ [____________________] [저장]      │
│                                                 │
└─────────────────────────────────────────────────┘
```
...
| 뷰 타입 | 표시 내용 |
|---------|----------|
| 일반 달력 (Calendar) | 운동 부위 아이콘/텍스트 |
| 상세 메모 (Memo) | 저장된 메모 텍스트만 |

메모 저장 규칙
- 입력 후 [저장] 클릭 시 DB에 저장
- 값이 있는 필드는 [저장] → [삭제] 버튼으로 변경
- [삭제] 클릭 시 해당 날짜 메모 삭제
- 빈 값으로 저장 시 기존 메모 삭제 처리

데이터 저장
- 테이블: `daily_memos` (모든 메모 통합 관리)
- 운동 안 한 날도 메모 작성 가능 (예: "휴식일", "컨디션 불량")
- `workout_sessions.notes`는 사용하지 않음 → 단일 데이터 소스 유지

3.4 운동 기록 페이지 (Recording View)
기능
...
3.1　2025-01-15　홈 탭 달력/메모 스위치 기능 추가 - 상세 메모 뷰 설계